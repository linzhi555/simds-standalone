...
// 				storage.Os.LogInfo(storage.Os, "task commit success", task.Worker, task)
// 			} else {
...
// 				storage.Os.LogInfo(storage.Os, "task commit fail ", storage.Workers[task.Worker], task)
// 			}

// 		case "TaskFinish":
// 			taskInfo := newMessage.Body.(TaskInfo)
// 			storage.Workers[newMessage.From].SubAllocated(taskInfo.CpuRequest, taskInfo.MemoryRequest)
// 			storage.Os.LogInfo(storage.Os, "received TaskFinish", newMessage.From, taskInfo)
...
// 			scheduler.Os.LogInfo(scheduler.Os, "received TaskDispense", task)
// 		case "TaskCommitFail":
...
// 			scheduler.Os.LogInfo(scheduler.Os, "reschedule task", task)
...
// 				panic(err)
// 			}
// 			scheduler.Os.LogInfo(scheduler.Os, "try to commit task allocate to globalStateStorage", task.Worker, task)
// 		} else {
// 			scheduler.WaitSchedule.InQueueFront(task)
// 			break
// 		}
